...
def create_html(final,filename,t):
    ...
    i = 0
    for image in range(t):
        if i % 6 == 0:
            html_op += ("<tr>")

        html_op += ("<td><div style='text-align:center'><img src='%s' width=200 height=200>ImageId:%s  Score:%f</div></td>" % (os.path.realpath(folder_path + final[image][0]), final[image][0] , final[image][1]))
        if (i+1)%6==0:
            html_op+=("</tr>")
        i+=1
    html_op += ("</tr></table>")
...
for layer in range(layers):
    ...
    for i,image in zip(technique_result['u'],images):
        hash_value=np.dot(random_vector[layer],np.array(i))
